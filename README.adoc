...
image:https://img.shields.io/badge/PowerShell-7.4-white[Static Badge]
image:https://img.shields.io/github/downloads/constup-foss/garbage-cleaner-powershell/total[GitHub Downloads (all assets, all releases)]
...
* *A PowerShell script, which can be run from the terminal or automated to be run by your own scripts*
* *Configurable with a simple JSON file*
* *It can clean files, directories and registry values*
* *It has a built-in cleanup list and supports your own custom cleanup lists as well*
...
Here is a preview of some commands. If you already have experience with running applications from the terminal, you
...
There are a number of tools available on the market, both free and commercial, for cleaning garbage files and registry
...
too much has different effects, depending on the tool: from logging you out of websites and destroying your sessions to
...
"software updater tools", which are harvesting the list of *all software that you have installed on your computer,
...
* You cannot (easily) process garbage which you or applications which you have written produce. For example, if you are
...
Download a release from https://github.com/constup-foss/garbage-cleaner-powershell/downloads/?tab=downloads and extract
...
[source,shell]
----
git clone git@github.com:constup-foss/garbage-cleaner-powershell.git
----
...
[source,shell]
----
git clone https://github.com/constup-foss/garbage-cleaner-powershell.git
----
...
[source,shell]
----
gh repo clone constup-foss/garbage-cleaner-powershell
----

=== Updating

To update, open the terminal and enter the directory where constUP Garbage Cleaner is located, then run:
...
=== Cleaning by using a custom set of rules
...
[IMPORTANT]
====
*All* constUP Garbage Cleaner commands can either use the default cleanup list or your custom one. To shorten
this documentation, we will not mention instructions for the custom cleanup list anymore - but it's available,
